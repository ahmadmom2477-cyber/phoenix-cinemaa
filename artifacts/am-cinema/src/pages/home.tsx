...
function getGreeting(isAr: boolean): { main: string; sub: string } {
  const h = new Date().getHours();
  if (h >= 5 && h < 12) return {
    main: isAr ? "صباح الخير 🌅" : "Good Morning ☀️",
    sub: isAr ? "ماذا تريد أن تشاهد اليوم؟" : "What would you like to watch today?",
  };
  if (h >= 12 && h < 17) return {
    main: isAr ? "مساء الخير ☀️" : "Good Afternoon 🌤",
    sub: isAr ? "ماذا تريد أن تشاهد اليوم؟" : "What would you like to watch today?",
  };
  if (h >= 17 && h < 21) return {
    main: isAr ? "مساء النور 🌇" : "Good Evening 🌆",
    sub: isAr ? "استمتع بأروع المحتوى الآن" : "Enjoy the best content now",
  };
  return {
    main: isAr ? "أهلاً بك 🌙" : "Welcome Back 🌙",
    sub: isAr ? "وقت مثالي لمشاهدة فيلم رائع" : "Perfect time for a great movie",
  };
}
  ...
  return (
    <div className="flex-1 flex flex-col pb-20">
      ...
      <div className="container max-w-7xl mx-auto px-4 md:px-8 mt-8 md:mt-12 space-y-12 md:space-y-20 relative z-20">
        ...
        >
          <h2 className="text-xl md:text-2xl font-serif font-bold text-white">{greeting.main}</h2>
