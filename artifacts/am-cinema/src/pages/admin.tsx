...
export default function Admin() {
  ...
  // ── Generate codes ──────────────────────────────────────────────────────────
  const showSuccess = (msg: string) => { setOpSuccess(msg); setOpError(""); setTimeout(() => setOpSuccess(""), 3500); };
  const showError = (msg: string) => { setOpError(msg); setOpSuccess(""); };
  ...
  const generateCodes = async () => {
    ...
    try {
      ...
      if (res.status === 429) { showError("تجاوزت الحد المسموح به، انتظر دقيقة ثم أعد المحاولة"); setGenerating(false); return; }
      if (!res.ok) { showError("فشل توليد الأكواد، حاول مجدداً"); setGenerating(false); return; }
      const data = await res.json() as { generated?: string[] };
      setGeneratedCodes(data.generated ?? []);
      setGeneratedLabel(genLabel);
      setGenLabel("");
      setGenCodeExpiry("");
      showSuccess(`تم توليد ${(data.generated ?? []).length} كود بنجاح`);
  ...
  const deleteCode = async (code: string) => {
    ...
    setOpError("");
    try {
      const res = await apiFetch(`/api/subscription/gifts/${encodeURIComponent(code)}`, { method: "DELETE" });
      if (res.status === 429) { showError("تجاوزت الحد المسموح به، انتظر قليلاً"); setDeletingCode(null); return; }
      if (!res.ok) { showError("فشل حذف الكود، حاول مجدداً"); setDeletingCode(null); return; }
  ...
  const revokeSessions = async (code: string) => {
    ...
    setOpError("");
    try {
      const res = await apiFetch(`/api/subscription/gifts/${encodeURIComponent(code)}/revoke-sessions`, { method: "POST" });
      if (res.status === 429) { showError("تجاوزت الحد المسموح به، انتظر قليلاً"); setRevokingCode(null); return; }
      if (!res.ok) { showError("فشل إلغاء الجلسات"); setRevokingCode(null); return; }
      showSuccess("تم إلغاء جميع الجلسات النشطة لهذا الكود");
      setTimeout(() => loadData(), 800);
    } catch { showError("خطأ في الاتصال"); }
            ...
            <AnimatePresence>
              {opError && (
                <motion.div key="err" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  className="flex items-center gap-2 px-4 py-3 rounded-xl bg-red-500/10 border border-red-500/25 text-sm text-red-400">
                  <span>⚠</span><span className="flex-1">{opError}</span>
                  <button onClick={() => setOpError("")} className="text-red-400/60 hover:text-red-400 ml-2">✕</button>
                </motion.div>
              )}
              {opSuccess && (
                <motion.div key="ok" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  className="flex items-center gap-2 px-4 py-3 rounded-xl bg-green-500/10 border border-green-500/25 text-sm text-green-400">
                  <span>✓</span><span>{opSuccess}</span>
                </motion.div>
              )}
            </AnimatePresence>
