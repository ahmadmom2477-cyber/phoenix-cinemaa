...
function formatSubLabel(sub: SubtitleOption, index: number, lang: "ar" | "en"): string {
  ...
  const KNOWN = [
    "DawoodTv", "Elzayady", "Islam El-Gezawy", "Anis Mansour",
    "Netflix", "🅽🅴🆃🅵🅻🅸🆇", "CimaNow", "Amazon Prime", "iTunes",
    "EgyBest", "الأصلية", "الاصليه", "اصليه", "النهائية", "الرسمية", "BluRay",
  ];
...
export default function Watch() {
  ...
  useEffect(() => {
    ...
    const autoApply = async () => {
      try {
        ...
        setSubtitleFileName(formatSubLabel(top, 0, lang));
  ...
  const applyAutoSubtitle = async (sub: SubtitleOption) => {
    ...
    try {
      ...
      setSubtitleFileName(formatSubLabel(sub, subIndex >= 0 ? subIndex : 0, lang));
      setSubtitleError("");
      setAutoSubStatus("applied");
  ...
  // Subtitle pause/resume toggle
  const toggleSubtitlePause = useCallback(() => {
    if (!subtitlePaused) {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      const elapsed = Date.now() - subtitleStartTimeRef.current - totalPausedMsRef.current + subtitleOffsetRef.current * 1000;
      const cue = subtitleCues.find((c) => elapsed >= c.start && elapsed <= c.end);
      setSubtitleCurrentText(cue?.text ?? "");
    }, 100);
    return () => { window.clearInterval(interval); };
  }, [clickGuardActive, subtitleCues, subtitlePaused]);
  ...
  return (
    <div className="flex-1 flex flex-col pt-20 md:pt-24 pb-8 md:pb-12 px-4 md:px-8 max-w-7xl mx-auto w-full">
      ...
      {/* Subtitle Status Bar — above player, shown when subtitle feature is enabled */}
      {autoSubEnabled && (
        <div className="mb-2 space-y-1.5">
          {autoSubStatus === "loading" && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-primary/5 border border-primary/20 text-xs">
              ...
              <span className="text-primary/80 font-medium">{t("جاري البحث عن ترجمة عربية... (قد يستغرق دقيقة أو دقيقتين)", "Searching for Arabic subtitle via Subscene… may take 1–2 minutes")}</span>
            </div>
          )}
          {autoSubStatus === "applied" && subtitleUrl && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-green-500/10 border border-green-500/20 text-xs">
              ...
              <span className="text-green-300 font-medium truncate flex-1">{subtitleFileName || t("ترجمة مطبقة", "Subtitle applied")}</span>
              <button onClick={clearSubtitle} className="text-white/40 hover:text-white/80 transition-colors shrink-0 ml-1">✕</button>
            </div>
          )}
          {autoSubStatus === "none" && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/3 border border-white/8 text-xs text-muted-foreground">
              <Globe size={11} className="shrink-0" />
              <span>{t("لم يُعثر على ترجمة عربية تلقائية — يمكنك الرفع يدوياً من لوحة الترجمة", "No Arabic subtitle found automatically — upload manually from the subtitle panel")}</span>
            </div>
          )}
          {autoSubOptions.length > 0 && autoSubStatus !== "applied" && (
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-[10px] text-muted-foreground uppercase tracking-widest shrink-0">{t("الترجمات:", "Subs:")}</span>
              {autoSubOptions.slice(0, 6).map((sub, idx) => (
                <button
                  key={sub.id}
                  onClick={() => applyAutoSubtitle(sub)}
                  disabled={applyingSubId === sub.id}
                  className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 text-xs text-white/70 hover:bg-primary/20 hover:border-primary/40 hover:text-primary transition-all disabled:opacity-50 max-w-[180px] truncate"
                >
                  {applyingSubId === sub.id ? <Loader2 size={10} className="animate-spin shrink-0" /> : null}
                  <span className="truncate">{formatSubLabel(sub, idx, lang)}</span>
                </button>
              ))}
            </div>
          )}
      ...
      {subtitleCues.length > 0 && !clickGuardActive && (
        <div className="flex items-center gap-1.5 mb-3 px-3 py-2 rounded-xl bg-white/5 border border-white/10 backdrop-blur-sm text-xs overflow-x-auto scrollbar-hide">
          ...
          {/* Pause/Resume subtitle timer */}
          <button
            onClick={toggleSubtitlePause}
            className={`px-2 py-0.5 rounded border transition-colors shrink-0 font-medium text-[11px] ${subtitlePaused ? "bg-amber-500/20 border-amber-500/40 text-amber-400" : "bg-white/5 border-white/10 text-muted-foreground hover:text-white hover:border-white/20"}`}
            title={t("إيقاف/تشغيل مؤقت لتوقيت الترجمة (اضغط عند الإيقاف المؤقت للفيديو)", "Pause/resume subtitle timer (press when you pause the video)")}
          >
            {subtitlePaused ? t("⏸ متوقفة", "⏸ Paused") : t("⏵ تشغيل", "⏵ Live")}
          </button>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-sm">
            ...
            <p className="text-xs text-muted-foreground mb-4">
              {autoSubEnabled
                ? t("ترجمات عربية تلقائية — اختر الأنسب لإصدار الفيديو", "Arabic subtitles — pick the one matching your video release")
