...
export default function Watch() {
  ...
  useEffect(() => {
    ...
    // Reclaim focus if something tries to open a new tab/popup
    const onVisibilityChange = () => {
      if (document.hidden) {
        setTimeout(() => { try { window.focus(); } catch {} }, 250);
      }
    };
    ...
    const domObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        m.addedNodes.forEach((node) => {
          ...
          const src = (node as HTMLIFrameElement | HTMLScriptElement).src ?? "";
