...
import { Film, Play, Star, TrendingUp, ChevronRight } from "lucide-react";
...
function MovieCard({ item, i }: { item: MediaItem; i: number }) {
  return (
    <motion.div
...
function GenreRow({ genreId }: { genreId: string }) {
  ...
  useEffect(() => {
    if (!genre) return;
    setLoading(true);
    fetch(`/api/tmdb/discover?type=movie&genre=${genreId}&page=1`)
      .then((r) => r.json())
      ...
      <div className="relative">
        <div className="flex overflow-x-auto gap-4 pb-6 snap-x snap-mandatory scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0">
          {loading
            ...
            : items.map((item, i) => <MovieCard key={item.imdbId} item={item} i={i} />)}
        </div>
...
const MOVIE_GENRES = ["action", "thriller", "crime", "drama", "scifi", "adventure", "horror", "comedy"];
        ...
        <div className="container max-w-7xl mx-auto px-4 md:px-8 relative z-10 w-full">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 className="text-3xl sm:text-5xl font-serif font-bold text-white mb-4 leading-tight">
              {t("استكشف الأفلام", "Explore Movies")}
            </h1>
            {hero && (
              <Link
          ...
          <div className="relative">
            <div className="flex overflow-x-auto gap-4 pb-6 snap-x snap-mandatory scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0">
              {isLoading
                ...
                : trending?.results?.map((movie, i) => <MovieCard key={movie.imdbId} item={movie} i={i} />)}
            </div>
            <div className="absolute top-0 right-0 bottom-0 w-20 bg-gradient-to-l from-background to-transparent pointer-events-none hidden md:block" />
          </div>
        </section>

        {MOVIE_GENRES.map((gId) => (
